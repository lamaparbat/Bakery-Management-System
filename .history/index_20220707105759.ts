...
console.log(process.env.HASH_SALT_ROUND)
...
server.post("/api/v4/customer/register", async (req: Request, res: Response) => {
  ...
  // encrypt password
  const encrypted_password = await bcrypt.hash(password, parseInt(process.env.HASH_SALT_ROUND));
  
  // db insertion
  try {
    const data = new customerModel({ username, email, encrypted_password });
    const result = await data.save();
  } catch (error) {
